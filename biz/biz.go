...
func GetDLJ(stub, pic, mov, apptaskid, source string) (string, string, error) {
	...
	if ok {
		url := "http://api-shoulei-ssl.xunlei.com/dlj_create"
		fileUrl := fmt.Sprintf("http://h5.m.xunlei.com/ios_page/live_photo/player.html?id=%d&apptaskid=%s&source=%s", insertId, apptaskid, source)
		fmt.Println(fileUrl)
		...
		payload := map[string]string{"accesskey": "ios.m.xunlei", "timestamp": currentTime, "nonce": nonce}
		sign := formatSign(url, "POST", paramsString, payload)
		url = url + fmt.Sprintf("?accesskey=%s&timestamp=%s&nonce=%s&sig=%s", "ios.m.xunlei", currentTime, nonce, sign)
		fmt.Println(url)
...
func rearangeWidthHeight(gcid string) {
	...
	fops := fmt.Sprintf("avthumb/mp4/s/%dx%d/stripmeta/1/vcodec/libx264|saveas/%s", width, height, storagePlace)
	fmt.Println(fops, "##########")
...
func formatUrl(gcid string) string {
	...
	QINIU_CDN_SECRET := "NWU4ZDBmMDQtMjE5ZC00NGZhLWIzZjgtMTE2OTYw"
	QINIU_LIVEPHOTO_DOWNLOAD_URI := "http://sl.wangpan.7niu.n0808.com/"
